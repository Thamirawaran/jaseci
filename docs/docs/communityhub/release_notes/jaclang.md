...
- **Support Shared Array Buffer**: Shared array buffer can be enabled in production by adding an enviornmental variable `security_headers = true` in production enviornmnet(`[environments.production]`).

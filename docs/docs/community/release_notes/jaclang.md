...
- **CFG Short-Circuit Evaluation**: Boolean `and`/`or` expressions in `if`/`elif`/`while` conditions are now split into separate CFG basic blocks reflecting short-circuit evaluation semantics. Each sub-condition gets its own node with distinct true/false edges, enabling accurate data flow analysis through complex boolean expressions including nested `(A and B) or C`, `A and (B or C)`, and chained `A and B and (C or D)` patterns.
